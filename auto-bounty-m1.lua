                    ...
                    if success and servers then
                        for k, v in pairs(servers) do
                            if k ~= game.JobId and v["Count"] <= 10 then 
                                ...
                                if serverBounty > (playerCount * 1200000) then
...
local function isValidTarget(p)
    ...
    if CONFIG.MinBounty > 0 and getBounty(p) < CONFIG.MinBounty then return false end
    return true
end
...
notify("Meyy Hub", "Loaded successfully", 3)task.spawn(function()
    local connection
    connection = RunService.RenderStepped:Connect(function()
---------------------------------------------------------
        local promptOverlay = game:GetService("CoreGui").RobloxPromptGui.promptOverlay
                    ...
                    if success and servers then
                        for k, v in pairs(servers) do
                            if k ~= game.JobId and v["Count"] <= 10 then 
                                local playerCount = v["Count"]
                                local serverBounty = v["Bounty"] or 0
                                if serverBounty > (playerCount * 2500000) then
                                    notify("Hop System", "Found server! Teleporting...", 3) 
                                    game.ReplicatedStorage.__ServerBrowser:InvokeServer("teleport", k) 
                                    task.wait()
                                end
...
-------------------------------------------------------------------------
    ...
    -- Thay thế toàn bộ code cũ bằng logic của Code 2
    local function FollowThread()
        ...
        -- Phần 1: Kiểm tra kích thước hitbox
        local function getH()
            return (plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")) 
                and plr.Character.HumanoidRootPart.Size.Magnitude or 0
        end
        ...
        -- Phần 2: Tìm kẻ địch và điểm Spawn
        local function getE()
        ...
        -- Phần 3: Hàm dịch chuyển (Tween TP)
        ...
        -- Phần 4: Logic tự động lướt (Lunge) và Target
        local function lunge()
            local h = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
            if not h then return end
            
            local cS = getH()
            if cS > 0 and math.abs(cS - bSize) > 0.5 then return end
