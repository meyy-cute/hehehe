...
function Library:SaveConfig(fileName, silent)
    ...
    if not silent then
        Library:Notify({Title = "Config System", Content = "Saved " .. fileName})
    end
end
...
function Library:LoadConfig(fileName, silent)
    ...
    if isfile(path) then
        ...
        if success and type(data) == "table" then
            ...
            Library.IsLoading = false
            if not silent then
                Library:Notify({Title = "Config System", Content = "Loaded " .. fileName})
            end
        else
            if not silent then
                Library:Notify({Title = "Config Error", Content = "Failed to parse JSON"})
            end
        end
    else
        if not silent then
            Library:Notify({Title = "Config Error", Content = "File not found"})
        end
    end
end
---------
...
function Library:Notify(options)
    ...
    local descText = options.Content or options.SubContent or ""
...
function Library:CreateWindow(options)
    local Window = {}
    options = options or {}
    local windowTitle = options.Title or "Hub Premium"
...
---------
 ...
 if Themes[CurrentTheme] and Themes[CurrentTheme].SearchIconColor then
    ...
    function Window:SelectTab(index)
        if TabsList[index] and TabsList[index].Select then
            TabsList[index].Select()
        end
    end
    ...
    function Window:AddTab(options)
        ...
        local tabIconId = options.Icon or ""
        local isFirstPage = (#TabsList == 0)
      ...
      ---------
        ...
        local function SelectTab()
            ...
            Library:Notify({Title = "Tab Selected", Content = name})
        end
        btn.MouseButton1Click:Connect(SelectTab)
        table.insert(TabsList, {Btn = btn, Bg = activeBg, Glow = activeGlow, Name = name, Select = SelectTab})
        ---------

        -- Add custom Page Title helper functions incase they are still needed
        function Tab:AddPageTitle(options)
            local text = options.Title or "Title"
            currentTitle = text
            currentSubTitle = ""
        ...
        -- Mapping original CreateDropdown logic to AddDropdown
        function Tab:_CreateSingleDropdown(options)
            local text = options.Title or "Dropdown"
            local desc = options.Description or ""
            local optionsList = options.Values or {}
...
---------
            ...
            local DropdownObj = { Value = default, Type = "Dropdown" }
            ...
            local function SetValue(val)
                ...
                if DropdownObj.Changed then DropdownObj.Changed(val) end
                Library:AutoSave()
            end

            Library.ConfigElements[flagId] = { Value = default, Set = SetValue }
            
            function DropdownObj:SetValue(v) SetValue(v) end
            function DropdownObj:OnChanged(cb) DropdownObj.Changed = cb end
            ...
            for _, opt in pairs(optionsList) do
                ...
                dummyBtn.MouseButton1Click:Connect(function()
                    ...
                    SetValue(opt)
                    Library:Notify({Title = "Selected Mode", Content = opt})
                end)
            end
        ...
        ---------
        function Tab:_CreateMultiDropdown(options)
            local text = options.Title or "Dropdown"
            local desc = options.Description or ""
            local optionsList = options.Values or {"Sample Mode 1", "Sample Mode 2"}
            local defaultSelections = options.Default or {}
            local callback = options.Callback or function() end
            local flag = options.Flag or text
...
---------
            ...
            local DropdownObj = { Value = defaultSelections, Type = "Dropdown" }
            ...
            local function SetValue(arr)
                ...
                DropdownObj.Value = saveArr
                if Library.ConfigElements[flagId] then
                    Library.ConfigElements[flagId].Value = saveArr
                end
                if callback then callback(selectedItems) end
                if DropdownObj.Changed then DropdownObj.Changed(selectedItems) end
                Library:AutoSave()
            end
        ...
        -- Master AddDropdown like Fluent
        function Tab:AddDropdown(options)
            if options.Multi then
                return self:_CreateMultiDropdown(options)
            else
                return self:_CreateSingleDropdown(options)
            end
        end
...
---------
            ...
            btnContainer.MouseButton1Click:Connect(function()
                ...
                Library:Notify({Title = "Button Clicked", Content = text})
            end)
        end
...
---------
            ...
            local ToggleObj = { Value = active, Type = "Toggle" }
            
            local function SetState(v)
                active = v
                switchData.Active = active
                ToggleObj.Value = active
                if Library.ConfigElements[flagId] then
                ...
                if ToggleObj.Changed then ToggleObj.Changed(active) end
                Library:AutoSave()
            end
            ...
            function ToggleObj:OnChanged(cb) ToggleObj.Changed = cb end

            toggleFrame.MouseButton1Click:Connect(function()
                SetState(not active)
                local status = active and "Enabled" or "Disabled"
                Library:Notify({Title = "Switch Toggled", Content = text .. ": " .. status})
            end)
       ...
       function Tab:AddLabel(options)
            ...
            local descText = options.Description or options.Content or ""
...
---------
            ...
            local function SetValue(val)
                val = math.floor(val / rounding) * rounding
                ...
                if callback then callback(val) end
                if SliderObj.Changed then SliderObj.Changed(val) end
            end

            Library.ConfigElements[flagId] = { Value = default, Set = SetValue }
            
            function SliderObj:SetValue(v) SetValue(v) end
            function SliderObj:OnChanged(cb) SliderObj.Changed = cb end
        ...
        ---------
   function Tab:AddCopy(options)
            local titleText = options.Title or "Copy"
            local descText = options.Description or ""
            local contentToCopy = options.Content or ""
            ...
            copyBtn.MouseButton1Click:Connect(function()
                ...
                Library:Notify({Title = "Copied Successfully", Content = titleText})
            end)
        end
        ---------
...
---------
    ...
    Library:Notify({Title = "Status", Content = "Hub Initialized!"})
    ...
    local InfoTab = Window:AddTab({ Title = "Information", Icon = "rbxassetid://132312092981034" })
    InfoTab:AddPageTitle({ Title = "Information" })
    InfoTab:AddCopy({ Title = "Server Discord", Content = "https://discord.gg/VnxbTKUe6T", Description = "Click to copy invite link" })
    InfoTab:AddLabel({ Title = "Designer Discord", Description = "aneex211" })
    InfoTab:AddLabel({ Title = "System", Description = "UI for meyy hub" })
    
    local ThemeTab = Window:AddTab({ Title = "Themes", Icon = "rbxassetid://136384267867469" })
    ThemeTab:AddPageTitle({ Title = "Themes" })
    ThemeTab:AddDropdown({
        Title = "Select Theme",
        Description = "Change UI colors",
        Values = {"Ocean", "Dream", "Dark"},
