...
Library.ConfigFolder = "KaitunHub_Configs"
...
local Themes = {
    ["Dark"] = {
        ...
        MainBgTrans = 0.3,
        ContainerBg = Color3.fromHex("#111111"), 
        ContainerTrans = 0.6,
        TextColor = Color3.fromHex("#FFFFFF"),
        DescTextColor = Color3.fromHex("#999999"),
...
function Library:SendNotification(titleText, descText)
    ...
    for i, notif in ipairs(Notifications) do
        if notif and notif.Parent then
            local newY = -20 - (i * 90)
            TweenService:Create(notif, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(1, -20, 1, newY)}):Play()
        end
    end
    ...
    if #Notifications > 5 then
        ...
        if oldNotif then
            TweenService:Create(oldNotif, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Position = UDim2.new(1, 300, 1, oldNotif.Position.Y.Offset)}):Play()
            task.delay(0.4, function() oldNotif.Parent:Destroy() end)
        end
    end
    
    local showTween = TweenService:Create(m2, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(1, -20, 1, -20)})
    showTween:Play()
...
function Library:CreateWindow(config)
    local Window = {}
    config = config or {}
    local windowTitle = config.Title or "Kaitun Hub"
    ...
    g.Name = "Kaitun_Premium_" .. math.random(100, 999)
    ...
    Mini3DIcon.Size = UDim2.new(0, 100, 0, 100)
    ...
    local coreLayers = {}
    local baseSizes = {25, 38, 52, 65, 78}    
    local baseTransparencies = {0.1, 0.3, 0.5, 0.7, 0.85}
    ...
    local scale3D, perspective3D = 35, 120
    local function project3D(v3)
        ...
        return Vector2.new(x * factor * scale3D + 50, -y * factor * scale3D + 50), z
    end
    ...
    MiniClickBtn.MouseButton1Click:Connect(function()
        ...
        local targetSize = isMax and UDim2.new(1, 0, 1, 0) or UDim2.new(0, 480, 0, 380)
...
---------
    ...
    Instance.new("UICorner", SnowContainer).CornerRadius = UDim.new(0, 12)
...
---------
    ...
    ToggleIcon.MouseButton1Click:Connect(function()
        ...
        if m.Size.X.Offset > 0 or m.Size.X.Scale > 0 then
            ...
            m.Visible = true
            local targetSize = isMini and UDim2.new(0, 480, 0, 380) or (isMax and UDim2.new(1, 0, 1, 0) or UDim2.new(0, 480, 0, 380))
...
---------
    ...
    local minBtn = CreateWinBtn("~", -90)
    local maxBtn = CreateWinBtn("⬚", -60)
    local closeBtn = CreateWinBtn("x", -30)
...
---------
    ...
    TopDivider.Position = UDim2.new(0, 10, 0, 45)
    ...
    LeftDivider.Size = UDim2.new(0, 1, 1, -55)
    LeftDivider.Position = UDim2.new(0, 135, 0, 50) 
...
---------
    ...
    function Window:CreateTab(name, isFirstPage, tabIconId)
        ...
        Instance.new("UICorner", activeBg).CornerRadius = UDim.new(0, 6)
        table.insert(UI_Elements.TabBackgrounds, activeBg)
        
        local activeGlow = Instance.new("Frame", activeBg)
        activeGlow.Size = UDim2.new(0, 3, 1, -8)
        activeGlow.Position = UDim2.new(0, 3, 0.5, 0)
        ...
        if tabIconId and tabIconId ~= "" then
            ...
            iconDisplay.Position = UDim2.new(0, 10, 0.5, 0)
        ...
        page.Name = name .. "Page"
        page.Size = UDim2.new(1, -145, 1, -60) 
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            page.Position = UDim2.new(0, 140, 0, 80) 
            ...
            Library:SendNotification("Selected", name)
        end)
---------

---------
        function Tab:CreateStatus(titleText, defaultVal)
            ...
            valLabel.Size = UDim2.new(0.5, -10, 1, 0)
            valLabel.Position = UDim2.new(0.5, 0, 0, 0)
            valLabel.BackgroundTransparency = 1
            valLabel.Font = Enum.Font.GothamBold
            valLabel.Text = tostring(defaultVal)
            valLabel.TextColor3 = Themes[CurrentTheme].ToggleActive
            valLabel.TextSize = 12
            valLabel.TextXAlignment = Enum.TextXAlignment.Right
            table.insert(UI_Elements.StatusValues, valLabel)
...
---------
        function Tab:CreateSwitch(text, default, desc, callback)
            local rowHeight = (desc and desc ~= "") and 50 or 38
            local row = Instance.new("Frame", page)
            ...
            label.Size = UDim2.new(0.7, -10, 0, 38)
            ...
            ball.Position = default and UDim2.new(1, -16, 0.5, 0) or UDim2.new(0, 2, 0.5, 0)
            ...
            local function SetState(v)
                ...
                TweenService:Create(ball, TweenInfo.new(0.2), {Position = active and UDim2.new(1, -16, 0.5, 0) or UDim2.new(0, 2, 0.5, 0)}):Play()
...
---------
        function Tab:CreateButton(text, desc, callback)
            ...
            local coreLayers = {}
            local baseSizes = {6, 10, 14, 18, 22, 26} 
...
---------
        function Tab:CreateDropdown(text, default, optionsList, callback)
            ...
            local label = Instance.new("TextLabel", row)
            label.Size = UDim2.new(0.5, -10, 0, 38)
            label.Position = UDim2.new(0, 10, 0, 0)
            label.BackgroundTransparency = 1
            label.Font = Enum.Font.GothamBold
            label.Text = text
            ...
            for _, opt in pairs(optionsList) do
                ...
                dummyBtn.Size = UDim2.new(1, -10, 0, 26)
                ...
                dummyBtn.MouseButton1Click:Connect(function()
                    isDropped = false
                    TweenService:Create(container, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(1, -4, 0, containerHeight)}):Play()
            ...
            dropBtn.MouseButton1Click:Connect(function()
                ...
                local targetHeight = isDropped and 130 or containerHeight
...
---------
    function Window:ApplyTheme(name)
        ...
        for _, val in pairs(UI_Elements.StatusValues) do
            if val and val.Parent then
                TweenService:Create(val, TweenInfo.new(0.3), {TextColor3 = t.ToggleActive}):Play()
            end
        end
    end
---------
...
---------
    m.Size = UDim2.new(0, 0, 0, 0)
    TweenService:Create(m, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 480, 0, 380)}):Play()
    Library:SendNotification("System", "Kaitun Loaded")
...
---------
local Window = Library:CreateWindow({Title = "Kaitun Premium"})

local FarmPage = Window:CreateTab("Auto Farm", true, "rbxassetid://132312092981034")
local levelStatus = FarmPage:CreateStatus("Current Level", "2550 (Max)")
local beliStatus = FarmPage:CreateStatus("Current Beli", "$15,400,200")

FarmPage:CreateDropdown("Select Weapon", "Melee", {"Melee", "Sword", "Fruit"}, function(val)
    print("Weapon:", val)
end)

FarmPage:CreateSwitch("Auto Farm Level", false, "Start fast leveling", function(state)
    print("Auto Farm Level:", state)
end)
FarmPage:CreateSwitch("Auto Farm Nearest", false, "", function(state) end)

local ItemPage = Window:CreateTab("Auto Items", false, "rbxassetid://136384267867469")
ItemPage:CreateStatus("Farming Target", "Godhuman")
ItemPage:CreateSwitch("Auto Godhuman", false, "Farm all materials needed", function(state) end)
ItemPage:CreateSwitch("Auto Cursed Dual Katana", false, "Do puzzles & farm", function(state) end)
ItemPage:CreateSwitch("Auto Soul Guitar", false, "Wait for full moon", function(state) end)
ItemPage:CreateButton("Teleport Mirage Island", "Find & TP to Mirage", function() end)

local SettingsPage = Window:CreateTab("Settings", false, "")
SettingsPage:CreateDropdown("Select Theme", "Dark", {"Dark", "Ocean", "Dream"}, function(selected)
