...
if getgenv().Config.BetaUi then
    ...
    MainTab:CreateParagraph("Status Log", "Initializing...")

    _G.updateStatus = function(text)
        pcall(function()
            local ui = getgenv().MainUI_Library
            if ui then
                local mainPage = ui:FindFirstChild("MainPage", true)
                if mainPage then
                    for _, v in pairs(mainPage:GetChildren()) do
                        if v:FindFirstChild("ParagraphTitle") and v.ParagraphTitle.Text == "Status Log" then
                            if v:FindFirstChild("ParagraphContent") then
                                v.ParagraphContent.Text = "Status: " .. tostring(text)
                            end
                        end
                    end
                end
    ...
    MainTab:CreateButton(
        ...
        function()
            ...
            Library:SendNotification("Server Info", "Job ID Copied!")
        end
    )
    ...
    MainTab:CreateButton(
        ...
        function()
            Library:SendNotification("Teleport", "Finding new server...")
            HopToServerByAPI("Fullmoon", 12, 2)
        end
    )
            ...
            pcall(function()
                for _, allyName in pairs(getgenv().Config["Allies Account"]) do
                    if not allyParagraphs[allyName] then
                        allyParagraphs[allyName] = { JobIdStr = nil }
                        AccountTab:CreateParagraph("Ally: " .. allyName, "Waiting for data...")
                        AccountTab:CreateButton("Join " .. allyName, "Teleport to this ally's server", function()
                            ...
                            if jobidnow then
                                Library:SendNotification("Teleport", "Joining ally...")
                                game:GetService("ReplicatedStorage").__ServerBrowser:InvokeServer("teleport", jobidnow)
                            else
                                Library:SendNotification("Error", "No valid server data!")
                            end
                        end)
                    end
                    ...
                    if dataplr and dataplr["data"] then
                        ...
                        local ui = getgenv().MainUI_Library
                        if ui then
                            local accountPage = ui:FindFirstChild("AccountPage", true)
                            if accountPage then
                                for _, v in pairs(accountPage:GetChildren()) do
                                    if v:FindFirstChild("ParagraphTitle") and v.ParagraphTitle.Text == "Ally: " .. allyName then
                                        if v:FindFirstChild("ParagraphContent") then
                                            v.ParagraphContent.Text = jobid .. " | " .. tostring(t-time) .. "s ago"
                                        end
                                    end
                                end
                            end
